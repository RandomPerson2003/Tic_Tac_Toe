  ...
  for (int i=0; i<noDraw.length; i++) {
    if ( noDraw[i]==true) {
      if (clickX[i]==true) {
        ...
        textDraw(X, Font, height, black, CENTER, CENTER, spaceX[i], spaceY[i], spaceWidth, spaceHeight);
        stroke(1);
        lines();
      } else if (clickO[i]==true) {
        noStroke();
        rect(spaceX[i], spaceY[i], spaceWidth, spaceHeight);
        textDraw(O, Font, height, black, CENTER, CENTER, spaceX[i], spaceY[i], spaceWidth, spaceHeight);
        stroke(1);
        lines();
      } else {
      }
